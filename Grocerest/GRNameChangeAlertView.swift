...
class GRNameChangeAlertView: UIView, UITextFieldDelegate {
    
    private let scalingView = UIView()
    private let alertBox = UIView()
    private let titleLabel = UILabel()
    private let nameField = GRNameChangeAlertView.createNameField()
    private let cancelButton = GRNameChangeAlertView.createButton(text: "Annulla")
    private let confirmButton = GRNameChangeAlertView.createButton(text: "Conferma")
    ...
    var name: String? {
        ...
        set(newValue) { nameField.text = newValue }
    }
    
    var placeholder: String? {
        get { return nameField.placeholder }
        set(newValue) { nameField.placeholder = newValue }
    }
        ...
        set (showOutline) {
            if showOutline {
                if nameField.text!.isEmpty {
                    nameField.layer.borderColor = UIColor.grocerestColor().cgColor
                    self.outline = true
                }
            } else {
                nameField.layer.borderColor = UIColor.grayText().cgColor
                self.outline = false
            }
        }
    }
        ...
        set (string) {
            if string == nil {
                ...
                alertBox.snp.remakeConstraints { make in
                    ...
                    make.height.equalTo(406.54 / masterRatio)
                }
                
                confirmButton.snp.remakeConstraints { make in
                    make.top.equalTo(nameField.snp.bottom).offset(48 / masterRatio)
                ...
                alertBox.snp.remakeConstraints { make in
                    ...
                    make.centerX.equalTo(self.snp.centerX)
                    make.width.equalTo(583 / masterRatio)
                    make.height.equalTo(502 / masterRatio)
                }
    ...
    private static func createNameField() -> UITextField {
        ...
        nameField.layer.borderColor = UIColor.grayText().cgColor
        nameField.leftView = imageContainer
    ...
    private func setupHierarchy() {
        ...
        backgroundTappableArea.addTarget(self, action: #selector(GRNameChangeAlertView.cancelButtonWasPressed(_:)), for: .touchUpInside)
        ...
        titleLabel.font = Constants.BrandFonts.ubuntuMedium18
        ...
        alertBox.addSubview(nameField)
        nameField.font = Constants.BrandFonts.avenirBook15
        nameField.textColor = UIColor.grocerestLightGrayColor()
        nameField.delegate = self
        nameField.snp.makeConstraints { make in
            make.top.equalTo(titleLabel.snp.bottom).offset(59 / masterRatio)
        ...
        cancelButton.addTarget(self, action: #selector(GRNameChangeAlertView.cancelButtonWasPressed(_:)), for: .touchUpInside)
        ...
        confirmButton.addTarget(self, action: #selector(GRNameChangeAlertView.confirmButtonWasPressed(_:)), for: .touchUpInside)
    }
